...
static int run_one_phase_in_current_process(
  ...
  for (int c = 0; c < execute_count; ++c) {
    ...
    if (!ok) {
      LOG(ERROR) << "[child] GOLDEN MISMATCH iter=" << c << " : " << why;
      return 2; // 用返回码告诉父进程失败
    }
  }
...
static int run_one_phase_in_child_process_and_wait(
  ...
  if (pid == 0) {
    // 子进程：建议重新初始化 glog（避免 fork 后的文件句柄/锁问题）
    // 如果你在 main 已经 InitGoogleLogging(argv,[object Object],)，这里也可以再调用一次
    // google::InitGoogleLogging("child");

    int rc = 0;
    try {
      rc = run_one_phase_in_current_process(
          xclbin_path, xmodel_path, input_file, golden_file, execute_count);
    ...
    // 关键：子进程不要 return 到父进程栈，直接 _exit
    _exit(rc);
  }
  ...
  if (WIFEXITED(status)) {
    ...
    if (code != 0) {
      LOG(ERROR) << "[parent] child exit code=" << code
                 << " xclbin=" << xclbin_path << " xmodel=" << xmodel_path;
    }
    return code;
  } else if (WIFSIGNALED(status)) {
    int sig = WTERMSIG(status);
    LOG(ERROR) << "[parent] child killed by signal=" << sig
               << " xclbin=" << xclbin_path << " xmodel=" << xmodel_path;
    return 128 + sig;
  }

  LOG(ERROR) << "[parent] child ended abnormally";
  return 99;
}
...
int main(int argc, char* argv[]) {
  // google::InitGoogleLogging(argv,[object Object],);
  ...
  const std::string golden_file      = "defog_golden.bin";

  for (int i = 0; i < 10000; ++i) {
    LOG(INFO) << "================ ITERATION=" << i << " ================";

    int rc1 = run_one_phase_in_child_process_and_wait(
        xclbin_dpu3d, xmodel_defog, defog_input_file, golden_file, 1);
    if (rc1 != 0) break;

    int rc2 = run_one_phase_in_child_process_and_wait(
        xclbin_dpu4k, xmodel_defog, defog_input_file, golden_file, 1);
    if (rc2 != 0) break;
  }
